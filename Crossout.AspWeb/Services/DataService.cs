...
namespace Crossout.AspWeb.Services
{
    public class DataService
    {
        ...
        {
            NPoco.Connection.Open();
            string query = "SELECT * FROM item LEFT JOIN itemlocalization ON itemlocalization.itemnumber = item.id LEFT JOIN rarity ON rarity.id = item.raritynumber WHERE item.removed = 0 AND item.meta = 0 AND item.sellprice <> 0 AND item.buyprice <> 0 AND itemlocalization.languagenumber = @0";
